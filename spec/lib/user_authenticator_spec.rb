...
describe UserAuthenticator do
  let(:user) { create :user, login: 'maxim', password: '123456789' }
  ...
  context "when initialized with login & password" do
    let(:authenticator) { described_class.new(login: 'maxim', password: '123456789') }
    let(:authenticator_class) { UserAuthenticator::Standard }

    describe "#initialize" do
      it "should initialize proper authenticator" do
        expect(authenticator_class).to receive(:new).with('maxim', 'secret')
        authenticator
      end
    end
